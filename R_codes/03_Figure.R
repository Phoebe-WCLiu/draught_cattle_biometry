...
ggplot(data = df, aes(x = MC_e1, y = MC_f4, shape = Sex, fill = as.factor(Type))) +
...
ggplot(data = df, aes(x = MT_e1, y = MT_f4, shape = Sex, fill = as.factor(Type))) +
  geom_star(aes(starshape=Sex), size=3.5) +
  coord_cartesian(ylim = c(0.55, 0.8)) +
  geom_ribbon(stat = 'function', fun = abline_fun(MT_LG_model1),
              mapping = aes(ymin = -Inf, ymax = after_stat(y)),
              fill = "#FC4E07", alpha = 0.07) +
  geom_ribbon(stat = 'function', fun = abline_fun(MC_LG_model1),
              mapping = aes(ymin = after_stat(y), ymax = Inf),
              fill = "#00AFBB", alpha = 0.07) +
...
ggplot(data = df, aes(x = MT_Bd_GL, y = MT_f4, shape = Sex, fill = as.factor(Type))) +
  geom_star(aes(starshape=Sex), size=3.5) +
  coord_cartesian(ylim = c(0.55, 0.8)) +
...
abline_fun <- function(model) {
  ...
  ggplot(data = df, aes(x = PH1_Bp_Dp, y = PH1_Bd_Glpe, shape = Sex, fill = as.factor(Type))) +
  geom_star(aes(starshape=Sex), size=3.5) +
  scale_shape_manual(values = c(21:25))+ 
...
abline_fun <- function(model) {
  ...
  ggplot(data = df, aes(x = PH2_Bp_GLpe, y = PH2_Bd_GLpe, shape = Sex, fill = as.factor(Type))) +
  geom_star(aes(starshape=Sex), size=3.5) +
  scale_shape_manual(values = c(21:25))+ 
